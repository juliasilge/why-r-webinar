...
meta() %>%
  meta_general(
    description = "Word embeddings statistically model language by mapping words or phrases to vectors of real numbers.",
    generator = "xaringan and remark.js"
  ) %>% 
  meta_social(
    title = "Understanding Word Embeddings",
    url = "https://juliasilge.github.io/why-r-webinar/",
    og_type = "website"
  )
```
...
```{r R.options = list(tibble.max_extra_cols=5, tibble.print_min=9, tibble.width=60)}
tidy_word_vectors
```
...
```{r echo=FALSE, fig.height=4, fig.width=7, out.width="95%"}
tidy_word_vectors %>%
...
```{r R.options = list(tibble.max_extra_cols=10, tibble.print_max=5, tibble.width=80)}
glove6b
```
