...
Our new function identifies skipgram windows in order to calculate the skipgram probabilities.
...
We can determine the word vectors from the PMI values using singular value decomposition.
...
## Each word can be represented as a numeric vector in this new, dense, 100-dimensional feature space. 
---

# Explore the CFPB word embeddings
...
tidy_word_vectors %>%
  ...
  facet_wrap(~dimension, scales = "free_y", ncol = 6) +
  coord_flip()
```
...
# How do we use these embeddings in modeling?
...
You can still use word embeddings for feature creation in modeling, just not embeddings that you determine yourself from your own dataset.
...
# Explore the GloVe word embeddings

Pre-trained word embeddings are trained on very large, general purpose English language datasets such as the text of Wikipedia plus other sources.
...
# How do we use pre-trained embeddings in modeling?
...
.footnote[
(Caliskan, Bryson, and Narayanan. "Semantics Derived Automatically from Language Corpora Contain Human-Like Biases." Science 356.6334 (2017): 183–186.)[http://arxiv.org/abs/1608.07187]
]
...
# How do biased embeddings affect models?
...
- Consider not using embeddings at all

--

- Find your own embeddings
...
- Newer work involves the idea of augmenting the training data with counterfactuals

-- 

- Other researchers suggest that corrections for fairness happen at the point of **decision**

-- 

- Evidence indicates that methods for debiasing allow the stereotypes to seep back in

.footnote[
(Gonen, Hila, and Yoav Goldberg. "Lipstick on a pig: Debiasing methods cover up systematic gender biases in word embeddings but do not remove them." arXiv preprint arXiv:1903.03862 (2019).)[http://arxiv.org/abs/1903.03862]
]
